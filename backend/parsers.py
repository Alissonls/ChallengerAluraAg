...
def parse_pdf(file_bytes: bytes, filename: str) -> str:
    ...
    try:
        ...
        # Fallback to string extraction if library fails or stream is partial
...
def parse_excel(file_bytes: bytes, filename: str) -> str:
    """Extract text from Excel .xlsx/.xls file bytes using openpyxl or pandas."""
    try:
        import openpyxl
...
def parse_pptx(file_bytes: bytes, filename: str) -> str:
    """Extract text from PowerPoint .pptx file bytes using python-pptx."""
    try:
        import pptx
...
def parse_json(file_bytes: bytes, filename: str) -> str:
    """Extract text from JSON file bytes, formatting keys and values nicely."""
    try:
        decoded = file_bytes.decode('utf-8', errors='ignore')
...
def parse_document(file_bytes: bytes, filename: str) -> Dict[str, Any]:
    ...
    Supports PDF, Word, Excel, PowerPoint, Markdown, CSV, JSON, HTML.
    """
